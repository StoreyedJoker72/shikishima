...
exports.run = async (client, message, Discord, args) => { // eslint-disable-line no-unused-vars
  ...
  try { // Put your Code Here.
    ...
    author = message.author.displayAvatarURL({size: 4096, format: "png"});

		const animatedGif = await canvacord.Canvas.spank(author, avatar);
		message.channel.send(new Discord.MessageAttachment(animatedGif, `${member.user.username}_spank.png`));
